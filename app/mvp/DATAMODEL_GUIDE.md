...
(issue #325 re-sourcing: `eventsForTicket` / `productScopesForTicket` over the
`ticketAdmittedSLAs` core in `resolve.js` — the PROJECT's contracts are the universe,
surviving the union of the (Customer) and (Applicant, Supplier) exact pairs; scopes
offer only what is CO-PACKAGED with the chosen event in a same payload, a payload with
an empty scope list widens to the event's full applicability (Q1), and the posture is
STRICT: no project / no surviving SLA = no options. On save the engine resolves and
