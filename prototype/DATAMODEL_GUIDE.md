...
| `computed: INHERITED-REQUIREMENTS(<eventField>) (display: requirementName)` | **Live requirements inheritance** (issue #226, Tickets.requirementName — replaces the #192 stored snapshot): the Active requirements AND-matched against the ticket's admitted payload chain — event → payloads under the APPLICANT → SUPPLIER contracts (issue #350, superseding the #325 project universe — `ticketAdmittedSLAs`, the exact (Applicant, Supplier) pair over every Active SLA, a blank side skipping its dimension; a snapshot ticket with zero covering payloads falls back to the pre-#325 customer-SLA path, the frozen-testdata tolerance rung) → product scopes (∩ the ticket's `productScopeID` when set) — plus the ticket's unit, that unit's served regions (`Business Units.regionID`), the ticket's inheritance parties and (issue #353 — `Requirements.branchID` activated) the ticket's PROJECT branch (blank context side skips the dimension). Requirement keys: a DECLARED dimension constrains, an undeclared one does not (sv106 refinement — a requirement defined only for a branch binds every ticket related to it via project/customer/applicant/supplier); Inactive requirements never inherit. Resolved live by `ticketRequirements` in `resolve.js` — a new aligned requirement surfaces on existing tickets with no re-seed. **The full rule→suite map lives in `INHERITANCE_CONTRACT.md`** — the executable contract for the real implementation. |
| `computed: TICKET-INPUTS(<processField>) (display: handoutName)` | **Customer-provided inputs of a ticket** (issue #280, Tickets.inputHandoutID — LIVE-derived, never stored): for each task of the ticket's processes, the live inherited requirement set plus the ticket's admitted product-scope context narrow the procedures to exactly ONE (#270 AND coverage; #332 scope gate — a procedure pinned to `productScopeID[]` scopes the ticket does not admit is no candidate, empty key = every scope Q1, and the task-level fallback with no ticket context stays ungated; a GAP/ambiguous task contributes nothing) and the inputs listed in that procedure's own `customerInputID[]` collect, deduped (issue #324 — the customer-input decision is PER PROCEDURE, subset of its Inputs; the handout-level `customerFlag` is retired, and a pre-sv77 procedure without the key falls back to it — frozen-testdata tolerance). Resolved by `ticketInputHandouts` in `resolve.js`; drives the Tickets Inputs tab (resolve overridden in `mapSubitem` — the via attr is never stored). |
| `computed: TICKET-PROCEDURE(<taskField>) (display: procedureRegistry)` | **The single procedure a ticket context selects for a task** (issue #270, Tasks.procedureRegistry — the Tickets Tasks tab's Procedure column): candidates = the task's APPROVED procedures (eligibility round sv103 — `procedureApproved` gates the dispatch, superseding the sv71 ungated decision) whose `requirementID[]` covers ALL the ticket's inherited requirements (AND) and whose `productScopeID[]` names an admitted scope (#332). Exactly one candidate → its registry renders as the info pill; zero → GAP; SEVERAL → GAP with the hover hint `Procedures redundancy <ids>` naming the collision (`ticketProcedureHint` in `resolve.js`, `cellHint` in `table.js`). `ticketProcedureCandidates`/`ticketProcedureForTask` in `resolve.js`; guarded by `test_engine_ticket_procedure_eligibility.mjs` in CI (the test-battery workflow). |
| `computed: PS-REQUIREMENTS(<inverseField>) (display: requirementName)` | **Comprehensive product-scope requirement set** (issue #288, legs inverted by #294, unit leg removed by #296 — Product Scopes.productScopeRequirements): the requirements **NAMING** the row (`Requirements.productScopeID`, the stored link declared on the REQUIREMENT — `<inverseField>` names that key; the form picker shows the `productScopeRegistry` code) ∪ those EXPLICITLY connected to the row's scope ∪ its product group — **three connection legs only**: a requirement sharing the row's `businessUnitID`/`regionID` does NOT attach through those dimensions (unit/region act purely as exclusion gates on the derived legs; unit-wide inheritance lives on the ticket/forecast chains under Q1). Still **no Q1 wildcard here**: a requirement with ALL keys blank attaches nowhere (the ticket chain above keeps Q1 and gains the `productScopeID` dimension under it — empty = all, non-empty = only the named product scopes; the Forecast Scopes compound chain carries the same leg). Derived legs skip Inactive requirements; a declared (named) link renders even Inactive — stored data wins, the pickers gate lifecycle. Resolved live by `productScopeRequirementRows` in `resolve.js`; feeds the REQUIREMENTS column and the Product Scopes → Requirements subitem tab (resolve overridden in `mapSubitem`, the #280 pattern — the via attr is never stored). (It also fed the Procedures Requirements picker via `requirementsForProductScopes` until issue #304 — that picker offers the UNIT-WIDE universe now: every Active requirement whose unit key is empty/names the selected unit and whose region key is empty/names a region the unit serves — `requirementsForUnit` in `forms.js`; unit/region stay exclusion gates, #296.) |
...
keys meaning "applies to all" — Q1), and the Tickets **Event** + **Product Scope** selects
(issue #350 re-sourcing, superseding the #325 project universe: `eventsForTicket` /
`productScopesForTicket` over the `ticketAdmittedSLAs` core in `resolve.js` — the
APPLICANT → SUPPLIER contracts are the universe: every Active SLA matching the exact
pair, a blank side skipping its dimension (blank both = every Active SLA; the Customer
only filters the Projects select); scopes
offer only what is CO-PACKAGED with the chosen event in a same payload, a payload with
an empty scope list widens to the event's full applicability (Q1), and no surviving
SLA = no options. On save the engine resolves and
STORES the matching `payloadID[]`/`slaID[]` — `applyDerivedUnits`). The SLA **Supplier** select joined them in the sv68
...
lands on segment-mates), the **Inputs/Outputs** pickers (`handoutsForDepartment`: the
department-admitted handouts, empty department list = offered everywhere — the
task-chain `handoutsForTask` narrowing stays on the Tasks form only), and the
**Requirements** FACETED control (`facetedRequirementOptions` + `mkFacetedChecks`:
one stored `requirementID[]`, four option partitions by what pins each requirement —
Product Scopes > Customers > Branches > Business Unit, most-specific facet wins, the
Customers facet grouped per customer and narrowed by the selected Branches).
The #359 round (REDEFINED same day) added the Tickets **Constraints** picker
(`constraintsForTicketUnit`: flagged `ticketSelectable` + Active requirements of the
ticket's business unit — `Requirements.businessUnitID` is MANDATORY since the
redefinition, demo rows seeded ALL units as the explicit Q1 spelling) — a pure FILTER
layer: `productScopesForTicket` gained an optional trailing `constraintIds` param that
keeps only the scopes whose comprehensive PS-REQUIREMENTS set covers ALL picks (AND);
the inheritance chain never reads the picks (the manual-union first cut was retired).
