...
of a branch purchases [Payloads](../operation/payload.md) from a department. Every contract
...
events covered by an SLA with that department are offered.
...
payloads, grouped by event); `Supplier *` — the supplying party, offered from **every**
customer grouped by type: an internal clinic can supply another unit's contract, or a
Supplier-type customer (labs, medical equipment) fulfils it; Activate. The events and
