...
!!! important "Customers connect to the operation through SLAs"
    ...
    the [Payloads](../operation/payload.md) the customer purchases, and tickets the customer
...
- [**Customers**](customers.md) — the registry of interested parties, typed Internal Client / External Client / Supplier. Geography is not registered here — a customer's city, country and region live on its Branches.
- [**SLA**](sla.md) — the Service Level Agreement (ISO §8.2) — the contract by which a customer of a branch purchases Payloads from a department. It is the gate on execution: when the customer opens a Ticket, only events…
