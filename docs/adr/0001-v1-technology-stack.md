...
The EDQMS prototype is a vanilla JavaScript SPA, with no build step, in-memory data, and a fixed login. v1 will be cloud-hosted (Azure, per the MVP architecture document), with real persistence, email authentication restricted to the `@northwind-energy.com` domain, and will be **open source**. Three forces shape the stack choice:
...
Third, the original UI requirement mentions **shadcn components**. shadcn/ui is React-only; the Vue equivalent is **shadcn-vue**, with the same philosophy (the component is copied into the project and stays under the team's control), built on Reka UI.

## Decision

Adopt the following stack for v1:

**Frontend.** Vue 3 with the Composition API and `<script setup>`, in **TypeScript**, bundled with Vite. UI components via **shadcn-vue** (Reka UI) on **Tailwind CSS**, applying the nance Design System tokens in the Tailwind theme, dark mode by default. Charts via shadcn-vue *charts* (Unovis); ECharts (`vue-echarts`) remains an exception for charts the default library does not cover well. The frontend is a **REST API client**, with no duplicated business logic.

**Backend.** Node.js with **Express** in TypeScript, exposing a versioned **REST API** (`/api/v1/...`) documented in OpenAPI. The backend hosts the **metadata engine** (ADR-0002), which resolves derived values, joins, cards, and reports on the server side — the client only renders. Authentication via **email OTP/magic link** with a domain allowlist enforced on the server (`@auth/express` from Auth.js or a minimal equivalent), session in an httpOnly cookie and store in Postgres; designed to be swapped for Entra ID at the future handoff without rewriting the application.
...
**Positive.** Vue 3 + SFC yields cohesive, small files, good for human review and for AI assistants. REST + OpenAPI is the most portable common denominator for contributors and for the handoff to IT. Drizzle keeps SQL visible and the schema derivable from the spec. The engine on the backend eliminates data leakage to the client and centralizes computation. Splitting into packages makes the engine testable without Vue or Postgres.
...
**React + Next.js (full-stack) + tRPC** — the design in the MVP architecture document. Discarded for contradicting the preference for Vue and for being more opinionated; tRPC also couples the client to TypeScript and is less familiar to open source contributors than REST.
